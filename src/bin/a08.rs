...
fn decode(digits: &[&[u8]], num: &[&[u8]]) -> Option<i32> {
    let tally = digits.iter().flat_map(|&word| word).counts();
    let b = tally.iter().find(|(_, &v)| v == 6)?.0;
    let e = tally.iter().find(|(_, &v)| v == 4)?.0;
    let f = tally.iter().find(|(_, &v)| v == 9)?.0;
    let one = digits.into_iter().find(|word| word.len() == 2)?;
    let c = one.iter().filter(|segment| segment != f).next()?;
    let parse_digit = |segments: &[u8]| -> i32 {
        match segments.len() {
            2 => return 1,
            4 => return 4,
